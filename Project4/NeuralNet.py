...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """
        newList = [1.0];
        for input in inActs: newList.append(input);
        return self.sigmoid(self.getWeightedSum(newList));
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        activation = self.sigmoidActivation(inActs);
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for index in range(len(self.weights)):
            newWeight = self.weights[index] + newList[index] * alpha * delta;
            weightList.append(newWeight);
            totalModification = totalModification + abs(newList[index] * alpha * delta);
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            activationList = [];
            for perceptron in layer: activationList.append(perceptron.sigmoidActivation(outputs[len(outputs) - 1]));
            outputs.append(activationList)
        return outputs;
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    error = 0;
                    for index in range(len(nextLayer)):
                        deltaTemp = nextLayer[index].weights[neuronNum + 1] * deltas[0][index];
                        error = error + deltaTemp;
                    delta = gPrime * error;
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        outputList = nnet.feedForward(example[0])[-1];
        correct = True;
        for index in range(len(outputList)):
            if round(outputList[index]) != example[1][index]: correct = False;
        if correct: testCorrect = testCorrect + 1;
        else: testError = testError + 1;
