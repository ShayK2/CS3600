    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items(): allPossible[newPos] += prob * self.beliefs[p];
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for index in range(self.numParticles):
            self.particleList.append(self.legalPositions[index % len(self.legalPositions)]);
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            for index in range(self.numParticles): self.particleList[index] = self.getJailPosition();
        else:
            for particle in self.particleList:
                weights[particle] += emissionModel[util.manhattanDistance(pacmanPosition, particle)];

            weights.normalize();
            if not weights.totalCount(): self.initializeUniformly(gameState);
            else:
                for index in range(self.numParticles): self.particleList[index] = util.sample(weights);
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particleList: distribution[particle] += 1;
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for index in range(self.numParticles): self.particles.append(particleList[index % len(particleList)]);
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for index in range(self.numGhosts):
            if noisyDistances[index] is None:
                for particle in range(self.numParticles):
                    self.particles[particle % len(self.particles)] = self.getParticleWithGhostInJail(
                        self.particles[particle % len(self.particles)], index);
        ...
        for particle in self.particles:
            ...
            for index in range(self.numGhosts):
                if noisyDistances[index] is not None:
                    distance = util.manhattanDistance(pacmanPosition, particle[index]);
        ...
        if not weights.totalCount():
            ...
            weights.normalize();
            for particle in range(self.numParticles): self.particles[particle] = util.sample(weights);
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for index in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), index,
    ...
    def getBeliefDistribution(self):
        distribution = util.Counter();
        for particle in self.particles: distribution[particle] += 1;
        distribution.normalize();
        return distribution;
