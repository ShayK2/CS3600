...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for livingGhost in livingGhostPositionDistributions:
            ghost = livingGhost.argMax();
            ghostProb = livingGhost[ghost];
            if ghostProb > bestProb: bestGhost, bestProb = ghost, ghostProb;

        minDist = 99999;
        bestAction = None;
        for action in legal:
            next = Actions.getSuccessor(pacmanPosition, action);
            distance = self.distancer.getDistance(next, bestGhost);
            if distance < minDist: minDist, bestAction = distance, action;
