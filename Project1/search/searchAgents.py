...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        """Returns the start state (in your state space, not the full Pacman state space)
            Tuple of zeroes represents whether or not the associated corner has been visited"""

        return self.startingPosition, (0, 0, 0, 0);

    def isGoalState(self, state):
        "Returns whether this search state is a goal state of the problem (all corners visited)"
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                # Add the successor to the visited corners list if its a corner
                if (nextx, nexty) in self.corners:
                    corners[self.corners.index((nextx, nexty))] = 1;

                # If the action is legal, add it to the successor list
                succ.append((((nextx, nexty), tuple(corners)), action, 1));
...
def cornersHeuristic(state, problem):
    ...
    # Make a list of corners left to visit, based on which state[1] values are 0
    remaining = [];
    for corner in problem.corners:
        if not state[1][problem.corners.index(corner)]: remaining.append(corner);
    ...
    while len(remaining) > 0:
        ...
        for corner in remaining:
            ...
            if not cornerDist or dist < cornerDist:
